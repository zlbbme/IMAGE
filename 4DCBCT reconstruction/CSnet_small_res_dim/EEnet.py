...
class SobelConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
        ...
        self.sobel_weight = nn.Parameter(torch.zeros(
            size=(out_channels, int(in_channels / groups), kernel_size, kernel_size),dtype=torch.float32), requires_grad=False)
...
if __name__ == '__main__':
    ...
    #     a=a.cuda()
        EE_Block = EdgeEnhanceREC().cuda() #实例化
    else:
        EE_Block = EdgeEnhanceREC()
    b = EE_Block(a)
