...
def ConvUnembed(InChannel_1, OutChannel_1, Kernal_1, InChannel_2, OutChannel_2, Kernal_2 ):  #Block/pBlock  to Max pooling and C
        ...
        #saver memeory and parameters
        DownBlock2 = nn.Sequential(
                nn.Conv2d(InChannel_1, InChannel_1//4, 1,1),    #STC后的卷积层为3*3卷积核，步进为1，填充为1，输出通道数为embed_dim // 4
                nn.LeakyReLU(negative_slope=0.2, inplace=True),   #LeakyReLU激活函数
                nn.Conv2d(InChannel_1//4, InChannel_1//4, 5,1),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(InChannel_1//4, InChannel_1 //12, 1,1), #1*1卷积核，步进为1，填充为0，输出通道数为embed_dim // 4
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(InChannel_1//12, InChannel_1 //12, 5,1), #1*1卷积核，步进为1，填充为0，输出通道数为embed_dim // 4
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(InChannel_1//12, OutChannel_1, 1, 1))     #3*3卷积核，步进为1，填充为1，输出通道数为embed_dim
        return DownBlock2
#写一个类，用于Unemmbedding
class Unembed(nn.Module):
    def __init__(self, embed_dim, OutChannel_1, Kernal_1, InChannel_2, OutChannel_2, Kernal_2):
        super(Unembed, self).__init__()
        self.embed_dim = embed_dim
        self.ConvUnembed = ConvUnembed(self.embed_dim, OutChannel_1, Kernal_1, InChannel_2, OutChannel_2, Kernal_2)
        
        self.patch_unembed = PatchUnEmbed(embed_dim=self.embed_dim,norm_layer=nn.LayerNorm)
    def forward(self, x):
        B, L, C = x.shape
        #print('x.shape',x.shape)
        self.x_size = (int(L**0.5),int(L**0.5))
        
        x = self.patch_unembed(x,self.x_size)
        #print('x.shape',x.shape)
        x = self.ConvUnembed(x)
        return x
...
class SwinTransformerBlock(nn.Module):   #sw transformer的基本模块，并不改变输入输出的维度，[B,H*W,C]->[B,H*W,C]
    ...
    def forward(self, x, x_size):  #[B,H*W,C]->[B,H*W,C]
        H, W = x_size
        B, L, C = x.shape
...
class SwinStage(nn.Module):
    ...
    def __init__(self, embed_dim, input_resolution,  num_heads, window_size, depth=1,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=PatchMerging,use_checkpoint=False,**kwargs):
        ...
        self.x_size = (int(self.input_resolution[0]),int(self.input_resolution[1])) #获取输入图像的H,W
        ...
        if downsample is not None:
            self.downsample = downsample(dim=self.embed_dim//2, norm_layer=norm_layer)
        else:
            self.downsample = None
        
    def forward(self, x):               #[B,H*W,embed_dim]->[B,H*W,embed_dim]
        
        #print('before swinstage',x.shape)
        self.x_size = (int(x.shape[1]**0.5),int(x.shape[1]**0.5))   #获取输入图像的H,W
        #print('x_size',self.x_size)
        if self.downsample is not None:
            x,self.x_size = self.downsample(x, self.x_size) #[B,H*W,embed_dim]->[B,H/2*W/2, 2*embed_dim]
        ...
        x = self.norm(x)
        #print('after swinstage',x.shape)
        return x

class Transformer(nn.Module):
    def __init__(self, img_size=512,embed_dim=96,num_layers=6, extract_layers=[1,2,3,4,5,6],norm_layer=nn.LayerNorm,**kwargs):  #[1，2，4，8，16，32]
        super().__init__()
        #path_merging layer
        self.embed_dim = embed_dim
        ...
        self.patch_embed    = PatchEmbed(embed_dim=embed_dim,norm_layer=norm_layer )   #patch_embed层，作用是将输入的图片分割成patch，然后将patch展平，再经过一个全连接层，得到一个向量
        self.patch_unembed  = PatchUnEmbed(embed_dim=embed_dim,norm_layer=norm_layer )   #patch_unembed层，作用是将patch展平的向量，经过一个全连接层，得到一个patch
        for i_layer in range(self.num_layers):
            layer = SwinStage(embed_dim= self.embed_dim*(2**i_layer), 
                            input_resolution=(img_size[0]/(2**i_layer), img_size[0]/(2**i_layer)), 
                            num_heads=6, window_size=8, depth=2, downsample=PatchMerging if (i_layer >0) else None )  #最后一个不带下采样PatchMerging
...
class PriorSwinNet(nn.Module):
    def __init__(self, img_size=512, patch_size=1, in_chans=1,
        ...
        self.unembed0 = Unembed(self.embed_dim,4,5,4,4,5)
        self.unembed1 = Unembed(self.embed_dim*2, 8, 5, 8,8, 5)  
        self.unembed2 = Unembed(self.embed_dim*4, 16, 5, 16, 16,5)
        self.unembed3 = Unembed(self.embed_dim*8, 32, 5, 32, 32,5)
        self.unembed4 = Unembed(self.embed_dim*16, 64, 5, 64, 64,5)
        self.unembed5 = Unembed(self.embed_dim*32, 128, 5, 128, 128,5)
    ...
    def forward(self, x):
        ...
        z0,z1,z2,z3,z4,z5 = self.transformer(x_first) 
        #print(z0.shape,z1.shape,z2.shape,z3.shape,z4.shape,z5.shape)  
        z0 = self.unembed0(z0)
        z1 = self.unembed1(z1)
        z2 = self.unembed2(z2)
        z3 = self.unembed3(z3)
...
if __name__ == '__main__':
    ...
    sw_Block = PriorSwinNet(img_size=512,embed_dim=24,num_layers=6, extract_layers=[1,2,3,4,5,6])
