...
## import testing data
from TestingDataset_CSnet_train import *
...
dataset_4DCBCT = TestingDataset_XCAT(
        '/Data/SaveBibMip-SX/100'   #原地址为：'./test_dataset0/'
...
root_save = r'./Results_Real/CS_small_res_multidim_epoch40_4st/overfit100'
...
for i, batch in enumerate( test_dataloader, 0 ):
        ...
        # #计算psnr和ssim
        # single_psnr = PSNR(output, labels)
        # single_ssim = SSIM(output, labels)
        # print('psnr:%d', single_psnr, 'ssim:%d', single_ssim)
        # total_psnr += int(single_psnr) 
        # total_ssim += int(single_ssim)
        ...
        # im = Image.fromarray(heatmap)
        # im.save( path + '/Processed' + str(SliceIndex+1) +'.png')
        print( path + '/Processed' + str(SliceIndex+1) +'.png' + ' saved successfully!')

print('processing finished!')
print('avg psnr:%d', total_psnr / (SliceNum * 10))
