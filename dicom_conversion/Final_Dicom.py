...
def TCIA_npy(patient_path,out_path):
    ...
    for root, dirs, files in os.walk(patient_path):
        ...
        if files[0].endswith(".DCM"):
            dicom_folder = root ; 
        out_mha_path = patient_path.replace('monaco_output_dicom','temp_mha')
...
def TCIA_2format(patient_path,out_path):
    #遍历patient_path下的所有文件
    for root, dirs, files in os.walk(patient_path):
        #如果files为空，则跳过
        if not files:
    ...
    for root, dirs, files in os.walk(out_mha_path):
        ...
        if files:
            for file in files:
                ...
                if 'CBCT' in mha_file :
                    print(file,'converting to png')
                    mha_conversion.convert_mha_to_png(mha_file, out_put_path)   #convert to png
                    out_put_path = out_put_path.replace('temp_png','temp_npy')
                    mha_conversion.convert_mha_to_npy(mha_file, out_put_path)   #convert to npy
                    continue
                else:
                    CT_mha_file = mha_file
                    CBCT_mha_file = mha_file.replace('CT','CBCT')
    ...
    print('png and npy have been conversion!')
